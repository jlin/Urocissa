...
fn read_exif(file_path: &Path) -> anyhow::Result<exif::Exif> {
    ...
    let exif = exif_reader
        ...
        .context(format!("Failed to read exif of file {:?}", file_path))?;
    Ok(exif)
}
...
pub fn generate_exif_for_video(database: &Database) -> anyhow::Result<BTreeMap<String, String>> {
    ...
    let output = Command::new("ffprobe")
        ...
        .output()
        .context(format!(
            "Spawn new command for ffprobe failed for {:?}",
            source_path
        ))?;

    if output.status.success() {
        let line = String::from_utf8(output.stdout).context(format!(
            "Failed to from vec<u8> to String for {:?}",
            source_path
        ))?;
        ...
        Err(anyhow::anyhow!(
            "ffprobe command failed with exit code {:?}",
