...
/// ---------- task model ----------
pub enum TaskState { Running(Instant), Done(f64) }
pub struct TaskRow { pub hash: ArrayString<64>, pub path: PathBuf, pub state: TaskState }
...
impl Dashboard {
    pub fn new() -> Self { Self { tasks: vec![], completed: VecDeque::new(), handled: 0, db_bytes: 0 } }

    pub fn add_task(&mut self, hash: ArrayString<64>, path: PathBuf) {
        if let Some(t) = self.tasks.iter_mut().find(|t| t.hash == hash) {
            t.path = path; t.state = TaskState::Running(Instant::now());
        } else {
            self.tasks.push(TaskRow { hash, path, state: TaskState::Running(Instant::now()) });
        }
    }
...
impl Component for Dashboard {
    fn draw_unchecked(&self, _: Dimensions, _: DrawMode) -> anyhow::Result<Lines> {
        ...
        let extra  = if remain > 0 { format!(" │  … remaining {remain}") } else { String::new() };
        let mut stats = format!("• Processed: {:<6} │ DB size: {:>8}{extra}", self.handled, human);
        ...
        if running_len >= max {
            ...
            for t in self.completed.iter().skip(start) {            // oldest → newest
                lines.push(vec![t.fmt()].try_into()?);
            }
        ...
        while lines.len() - 3 < max {                               // minus rule+stats+rule already added
