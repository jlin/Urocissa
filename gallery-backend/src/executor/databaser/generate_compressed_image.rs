use crate::{
    executor::databaser::{
        generate_dynamic_image::generate_dynamic_image, generate_exif::generate_exif,
    },
...
use super::{fix_orientation::fix_orientation, small_width_height};
...
pub fn generate_compressed_image(
    ...
    let dynamic_image = if let Some(dyn_image) = dynamic_image_orientated {
        ...
        generate_exif(&database);
        let mut dyn_img = generate_dynamic_image(database)?;
        fix_orientation(database, &mut dyn_img);
        dyn_img
    };
