...
fn decode_image(file_path: &PathBuf) -> Result<DynamicImage, Box<dyn Error>> {
...
fn image_crate_decoder(file_in_memory: &Vec<u8>) -> Result<DynamicImage, Box<dyn Error>> {
    let dynamic_image = image::load_from_memory(file_in_memory)?;
    Ok(dynamic_image)
}

fn zune_jpeg_decoder(file_in_memory: &Vec<u8>) -> Result<DynamicImage, Box<dyn Error>> {
